...
const filterCar = (avail, tanggal, waktu, kapasitas) => {
  ...
  let newCar = AllCar.filter((car) => car.available.toString() == avail && new Date(car.availableAt) < new Date(tanggal) && new Date(car.availableAt).getHours() >= waktu && car.capacity == kapasitas);
  const body = document.getElementById('cm-cards');
  body.innerHTML = '';
  ...
  newCar.forEach((data) => {
    ...
    Car.innerHTML = `
        <div class="card cm-content">
        <img src="${data.image}" alt="" class="cars">
...
const filterCarTwo = (avail, tanggal, waktu) => {
  console.log(new Date('2022-03-23T15:49:05.563Z'));
  let newCar = AllCar.filter((car) => car.available.toString() == avail && new Date(car.availableAt) < new Date(tanggal) && new Date(car.availableAt).getHours() >= waktu);
  const body = document.getElementById('cm-cards');
  body.innerHTML = '';
